...
export function isExitPromptError(error) {
  ...
  return (
    ...
    (errorMessage && errorMessage.includes("SIGINT"))
  );
}
...
function showValidationError(
  ...
  if (errorMessage) {
    ...
    console.error(
      chalk.red(
        `Error: '${value}' is not a valid ${typeLabel} for '${parentCommand}'.`,
      ),
    );
  } else {
  ...
  options.forEach((option) => {
    console.error(
      `  ${chalk.yellow(option.value)} - ${chalk.blue(option.description)}`,
    );
  });
...
function handleTooManyArgs(commandName, maxArgs, parentCommand) {
  ...
  if (cmdIndex === -1) {
    ...
    console.error(
      chalk.red(
        `Error: No more arguments are required after '${commandName}'.`,
      ),
    );
  } else {
    const providedArgs = args.slice(cmdIndex + 1);
    const lastValidArg =
      maxArgs > 0 && providedArgs.length > maxArgs
        ? providedArgs[Math.min(maxArgs - 1, providedArgs.length - 2)] ||
          commandName
        : commandName;
    console.error(
      chalk.red(
        `Error: No more arguments are required after '${lastValidArg}'.`,
      ),
    );
  }
  process.exit(1);
  ...
  return {
    writeErr: (str) => {
      if (
        ...
        if (
          validOptions &&
          (str.includes("unknown command") || str.includes("unknown option"))
        ) {
          const args = process.argv.slice(2);
