...
export function whitelistCommand() {
  ...
  cmd.description(
    "Whitelist an agent for local environment (whitelist_agent_for_local)",
  );
  ...
  cmd.action(async () => {
    try {
      ...
      if (deployment.environment !== "local") {
        console.error(
          chalk.red(
            "Error: whitelist_agent_for_local is only valid when environment is local.",
          ),
        );
        console.error(
          chalk.red(`Current environment is: ${deployment.environment}`),
        );
        process.exit(1);
      }
      ...
      if (!deployment.whitelist_agent_for_local) {
        console.error(
          chalk.red(
            "Error: whitelist_agent_for_local is not configured in deployment.yaml",
          ),
        );
        console.error(
          chalk.yellow(
            "Please add a whitelist_agent_for_local section to your deployment.yaml file.",
          ),
        );
        process.exit(1);
      }
      ...
      if (!credentials) {
        console.log(
          chalk.red(
            `Error: No master account found for ${deployment.network} network.`,
          ),
        );
        console.log(
          chalk.yellow('Please run "shade auth set" to set master account.'),
        );
        process.exit(1);
      }
      ...
      const argsStr =
        typeof whitelistCfg.args === "string"
          ? whitelistCfg.args
          : JSON.stringify(whitelistCfg.args);
      ...
      const resolvedArgs = agentAccountId
        ? replacePlaceholders(whitelistCfg.args, {
      ...
      try {
        ...
        if (result && result.final_execution_outcome) {
          const success = checkTransactionOutcome(
            result.final_execution_outcome,
          );
          if (!success) {
            console.error(
              chalk.red(
