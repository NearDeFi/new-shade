...
export async function createAccount() {
  ...
  if (totalBalance < requiredBalance) {
    console.log(
      chalk.red(
        `Error: You need to fund your master account ${masterAccount.accountId}`,
      ),
    );
    console.log(
      chalk.yellow(
...
async function innerDeployCustomContractFromWasm(wasmPath) {
  ...
  try {
    ...
    console.log(
      chalk.red(`Error deploying the custom contract from WASM: ${e.message}`),
    );
    process.exit(1);
  }
}
...
function resolveWasmPath(absoluteSourcePath) {
  ...
  if (!fs.existsSync(wasmPath)) {
    console.log(
      chalk.red(
        `WASM not found at ${wasmPath} make sure the contract build produced this file.`,
      ),
    );
    process.exit(1);
  }
...
export async function deployCustomContractFromSource() {
  ...
  try {
    ...
    console.log(
      chalk.red(
        `Error building/deploying the custom contract from source: ${e.message}`,
      ),
    );
    process.exit(1);
  }
...
export async function deployCustomContractFromGlobalHash() {
  ...
  try {
    ...
    console.log(
      chalk.red(
        `Error deploying the custom contract from global hash: ${e.message}`,
      ),
    );
    process.exit(1);
  }
