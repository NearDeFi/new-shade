...
describe('ShadeClient', () => {
  ...
  function setupClientMocks(options?: {
    ...
    vi.mocked(getDstackClient).mockResolvedValue(options?.dstackClient || undefined);
  ...
  describe('create', () => {
    ...
    it('should throw error if validation fails', async () => {
      vi.mocked(validateShadeConfig).mockRejectedValue(new Error('Invalid config'));
  ...
  describe('balance', () => {
    it('should return account balance in NEAR', async () => {
      ...
      (mockAccount.getBalance as ReturnType<typeof vi.fn>).mockResolvedValue(balance);
      
      const client = await ShadeClient.create({ rpc: mockProvider });
      const result = await client.balance();
      
      expect(createAccountObject).toHaveBeenCalledWith(testAccountId, mockProvider);
    ...
    it('should return 0 if account does not exist', async () => {
      ...
      (mockAccount.getBalance as ReturnType<typeof vi.fn>).mockRejectedValue(error);
  ...
  describe('register', () => {
    ...
    it('should throw error if attestation fetch fails', async () => {
      setupClientMocks();
      vi.mocked(internalGetAttestation).mockRejectedValue(new Error('Network error'));
  ...
  describe('view', () => {
    ...
    it('should throw error if agentContractId is not configured', async () => {
      ...
      await expect(client.view({ methodName: 'test', args: {} })).rejects.toThrow(
        'agentContractId is required for view calls'
      );
    });
  });
  ...
  describe('call', () => {
    it('should call function on contract', async () => {
      ...
      (mockAccount.callFunction as ReturnType<typeof vi.fn>).mockResolvedValue(mockResponse);
    ...
    it('should rotate keys on subsequent calls', async () => {
      ...
      expect(getAgentSigner).toHaveBeenNthCalledWith(1, [testPrivateKey, secondKey], 0);
      // After first call, currentKeyIndex is updated to 1 (from getAgentSigner return)
      expect(getAgentSigner).toHaveBeenNthCalledWith(2, [testPrivateKey, secondKey], 1);
    });
    ...
    it('should throw error if agentContractId is not configured', async () => {
      ...
      await expect(client.call({ methodName: 'test', args: {} })).rejects.toThrow(
        'agentContractId is required for call functions'
      );
    });
  });
