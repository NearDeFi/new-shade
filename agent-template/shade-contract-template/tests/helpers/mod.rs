    ...
    pub async fn deploy_contract(
        ...
        let contract_id = account_id.unwrap_or_else(|| {
            format!("contract.{}", genesis_account_id).parse().unwrap()
        });
            ...
            match result
            {
                ...
                Err(e) => {
                    ...
                    if error_str.contains("408") || error_str.contains("timeout") || error_str.contains("Timeout") || error_str.contains("TransportError") {
                        println!("Deployment attempt {} timed out (408), retrying... (attempt {}/3)", attempt, attempt);
